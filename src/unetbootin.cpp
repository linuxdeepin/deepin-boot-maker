...
unetbootin::unetbootin(QObject *parent)
    : QObject(parent)
{
}
...
{
    qDebug()<<(tr("Locating kernel file in %1").arg(archivefile));
	if (issalt) {
...
bool unetbootin::extractkernel(QString archivefile, QString kernoutputfile, QPair<QStringList, QList<quint64> > archivefileconts)
{
	...
	if (kfloc == "")
		return false;
    qDebug()<<(tr("Copying kernel file from %1").arg(kfloc));
...
{
    qDebug()<<(tr("Locating initrd file in %1").arg(archivefile));
...
{
    qDebug()<<(tr("extractinitrd  in %1").arg(archivefile));
	QString kfloc = locateinitrd(archivefile, archivefileconts);
    qDebug()<<(tr("locateinitrd end"));
	if (kfloc == "")
		return false;
    qDebug()<<(tr("Copying initrd file from %1").arg(kfloc));
	return extractfile(kfloc, kernoutputfile, archivefile);
}

QString unetbootin::extractcfg(QString archivefile, QStringList archivefileconts)
{
    qDebug()<<(tr("Extracting bootloader configuration"));
	QString saltpcfg;
	QString grubpcfg;
...
void unetbootin::extractiso(QString isofile)
{
    ...
    if (!sdesc2String.contains(trcurrent))
	{
        ...
        sdesc2String = (QString("<b>%1 %2</b>").arg(sdesc2String).arg(trcurrent));
	}
	...
	if (listfilesizedirpair.first.first.size() == 1)
	{
        ...
        qDebug()<<(tr("<b>Extracting compressed iso:</b> %1").arg(subarchivename));
...
void unetbootin::extractiso_krd10(QString isofile)
{
	...
	{
        sdesc1String = (QString(sdesc1String).remove("<b>").replace(trcurrent+"</b>", trdone));
        sdesc2String = (QString("<b>%1 %2</b>").arg(sdesc2String).arg(trdone));
	}
	tprogress->setValue(0);
	QPair<QPair<QStringList, QList<quint64> >, QStringList> listfilesizedirpair = listarchiveconts(isofile);
    ...
    qDebug()<<(QString("Copying %1 to %2").arg(isofile).arg(QString("%1rescue%2rescue.iso").arg(targetDrive).arg(QDir::toNativeSeparators("/"))));
...
//	qDebug()<<(tr("<b>Downloaded:</b> 0 bytes"));
...
void unetbootin::dlprogressupdate(int dlbytes, int maxbytes)
{
 ...
 if(oldsec != time.second())
 {
   ...
   // display the downloaded size with suffix
     qDebug()<<(tr("<b>Downloaded:</b> %1 of %2").arg(displayfisize(dlbytes)).arg(displayfisize(maxbytes)));
 }
}
...
void unetbootin::cpprogressupdate64(qint64 dlbytes, qint64 maxbytes)
{
 ...
 if(oldsec != time.second())
 {
   oldsec = time.second();
	 ...
	 tprogress->setMaximum(maxbytes);
   // display the downloaded size with suffix
     qDebug()<<(tr("<b>Copied:</b> %1 of %2").arg(displayfisize(dlbytes)).arg(displayfisize(maxbytes)));
 }
}
...
//	if (!ldfDirStringUrl.endsWith('/'))
//		ldfDirStringUrl += '/';
//	qDebug()<<(tr("Searching in <a href=\"%1\">%1</a>").arg(ldfDirStringUrl));
...
QString unetbootin::getlabel(QString voldrive)
{
	...
	voldrive.append("\\");
	wchar_t vollabel[50];
    GetVolumeInformation(LPWSTR(voldrive.utf16()), vollabel, 50, NULL, NULL, NULL, NULL, 0);
...
QString unetbootin::getuuid(QString voldrive)
{
    ...
    GetVolumeInformation(LPWSTR(voldrive.utf16()), NULL, 0, &volserialnum, NULL, NULL, NULL, 0);
    if (!(volserialnum >= 1))
	{
		return "None";
	}
...
void unetbootin::runinst()
{
    ...
    sdesc1String = (QString("<b>%1 %2</b>").arg(sdesc1String).arg(trcurrent));
	tprogress->setValue(0);
    installType = tr("USB Drive");
    targetDrive = usbDriverPath;
    persistenceSpaceMB = 0;
	QString ginstallDir;
	QString installDir;
    ...
    {
        return;
        //downloadfile(isoImagePath, isotmpf);
        //extractiso(isotmpf);
    }
    if (QFile::exists(QString("%1sevnz.exe").arg(ubntmpf)))
    {
    ...
    if (!sdesc1String.contains(trdone))
	{
        sdesc1String = (QString(sdesc1String).remove("<b>").replace(trcurrent+"</b>", trdone));
	}
    if (sdesc2String.contains(trcurrent))
	{
        sdesc2String = (QString(sdesc2String).remove("<b>").replace(trcurrent+"</b>", trdone));
	}
	else
	{
        sdesc2String = (QString("%1 %2").arg(sdesc2String).arg(trdone));
    }
	tprogress->setValue(0);
...
//void unetbootin::copyDir(const QString &dirSrc, const QString &dirDest)
//{
//#ifdef Q_OS_UNIX
//    callexternapp("sync", "");
//#endif
...
bool unetbootin::rmDir(const QString &fn)
{
    bool result = true;
    QDir dir(fn);
